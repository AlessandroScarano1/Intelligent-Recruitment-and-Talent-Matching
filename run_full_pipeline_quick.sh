#!/bin/bash
# Full pipeline execution script (QUICK MODE)
# Runs CV ingestion -> Job ingestion -> Training (quick) -> Index building (quick)
...
LOG_FILE="$OUTPUT_DIR/quick_pipeline_${TIMESTAMP}.log"

# redirect all output to both console and log file
exec > >(tee -a "$LOG_FILE") 2>&1

echo "FULL PIPELINE EXECUTION (QUICK MODE)"
echo ""
echo "Quick mode settings:"
echo "  - Training: single run with best hyperparams"
echo "  - Index: 10,000 job sample instead of full 1.35M"
echo ""
echo "Pipeline order:"
echo "  1. CV Ingestion (scripts 01-05)"
echo "  2. Job Ingestion (scripts 01-06)"
echo "  3. Training (scripts 07-10, quick mode)"
echo ""
...
# PHASE 3: TRAINING PIPELINE (QUICK MODE)
...
echo ""
echo "PIPELINE EXECUTION COMPLETE (QUICK MODE)"
echo ""
...
echo "  - Indexes: training/output/indexes/ (quick: 10k jobs)"
echo "  - Log file: $LOG_FILE"
echo ""
