#!/bin/bash
# Hybrid pipeline execution script
# Runs CV ingestion -> Job ingestion -> Training (quick, no sweep) -> Full index (1.35M jobs)
# Best for: Fast training with production-ready full index
...
LOG_FILE="$OUTPUT_DIR/quick_train_full_index_${TIMESTAMP}.log"

# redirect all output to both console and log file
exec > >(tee -a "$LOG_FILE") 2>&1

echo "PIPELINE EXECUTION (QUICK TRAINING + FULL INDEX)"
echo ""
echo "Configuration:"
echo "  - CV Ingestion: Full (4,817 CVs)"
echo "  - Job Ingestion: Full (1.35M jobs)"
echo "  - Training: Quick mode (single run, no sweep)"
echo "  - Index: Full mode (all 1.35M jobs)"
echo ""
echo "Pipeline order:"
echo "  1. CV Ingestion (scripts 01-05)"
echo "  2. Job Ingestion (scripts 01-06)"
echo "  3. Training (scripts 07-10, quick training + full index)"
echo ""
...
# PHASE 3: TRAINING PIPELINE (QUICK TRAIN + FULL INDEX)

echo ""
echo "PHASE 3: TRAINING PIPELINE (QUICK TRAIN + FULL INDEX)"
...
echo ""
echo "[COMPLETE] Training pipeline finished (quick train + full index)"
echo ""
...
echo "  [✓] Training (quick mode: single run)"
echo "  [✓] Index (full mode: 1.35M jobs)"
echo ""
echo "End time: $(date)"
echo ""
...
echo "  - Indexes: training/output/indexes/ (full: 1.35M jobs)"
echo "  - Log file: $LOG_FILE"
echo ""
